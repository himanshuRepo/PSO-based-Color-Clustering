...
function [idx,cc]=pso(data,numClusters)

CostFunction=@(x) clusteringCost(x,data);        % Cost Function

[m,n]=size(data);

nVar=n*numClusters;            % Number of Decision Variables
...
VarMin=repmat(VarMin,[1,numClusters]);
VarMax=repmat(VarMax,[1,numClusters]);
